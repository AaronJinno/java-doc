import{_ as n,o as s,c as a,e as t}from"./app-acb3e2bc.js";const e={},i=t(`<div class="hint-container danger"><p class="hint-container-title">默认情况</p><p>默认情况下，MongoDB实例启动运行时是没有启用用户访问权限控制的。<br> 也就是说，可以随意连接到实例进行各种操作，MongoDB不会对连接客户端进行用户验证，这是非常危险的。</p></div><h4 id="_1-安全保障手段" tabindex="-1"><a class="header-anchor" href="#_1-安全保障手段" aria-hidden="true">#</a> 1. 安全保障手段</h4><ul><li>使用新的端口，默认的27017端口如果一旦知道了ip就能连接上，不太安全</li><li>设置mongodb的网络环境，最好将mongodb部署到公司服务器内网，这样外网是访问不到的。公<br> 司内部访问使用vpn等。</li><li>开启安全认证。认证要同时设置服务器之间的内部认证方式，同时要设置客户端连接到集群的账号<br> 密码认证方式。</li></ul><h4 id="_2-启动访问控制" tabindex="-1"><a class="header-anchor" href="#_2-启动访问控制" aria-hidden="true">#</a> 2. 启动访问控制</h4><p>为了强制开启用户访问控制(用户验证)，则需要在MongoDB实例启动时使用选项 --auth 或在指定启动<br> 配置文件中添加选项 auth=true。</p><p>请选择在配置文件中添加选项，因为启动时加选项毫无意义，别人可以选择不加。</p><h4 id="_3-概念" tabindex="-1"><a class="header-anchor" href="#_3-概念" aria-hidden="true">#</a> 3. 概念</h4><h5 id="_3-1-启动访问控制" tabindex="-1"><a class="header-anchor" href="#_3-1-启动访问控制" aria-hidden="true">#</a> 3.1 启动访问控制</h5><p>MongoDB使用的是基于角色的访问控制(Role-Based Access Control,RBAC)来管理用户对实例的访问。</p><p>通过对用户授予一个或多个角色来控制用户访问数据库资源的权限和数据库操作的权限，在对用户分配<br> 角色之前，用户无法访问实例</p><h5 id="_3-2-角色" tabindex="-1"><a class="header-anchor" href="#_3-2-角色" aria-hidden="true">#</a> 3.2 角色</h5><p>在MongoDB中通过角色对用户授予相应数据库资源的操作权限，每个角色当中的权限可以显式指定，<br> 也可以通过继承其他角色的权限，或者两都都存在的权限。</p><h5 id="_3-3-权限" tabindex="-1"><a class="header-anchor" href="#_3-3-权限" aria-hidden="true">#</a> 3.3 权限：</h5><p>权限由指定的数据库资源(resource)以及允许在指定资源上进行的操作(action)组成。</p><ul><li>资源(resource)包括：数据库、集合、部分集合和集群；</li><li>操作(action)包括：对资源进行的增、删、改、查(CRUD)操作。</li></ul><p>在角色定义时可以包含一个或多个已存在的角色，新创建的角色会继承包含的角色所有的权限。在同一<br> 个数据库中，新创建角色可以继承其他角色的权限，在 admin 数据库中创建的角色可以继承在其它任意数据库中角色的权限。</p><h4 id="_4-查看角色权限" tabindex="-1"><a class="header-anchor" href="#_4-查看角色权限" aria-hidden="true">#</a> 4. 查看角色权限</h4><h5 id="_4-1-查询所有角色权限-仅用户自定义角色" tabindex="-1"><a class="header-anchor" href="#_4-1-查询所有角色权限-仅用户自定义角色" aria-hidden="true">#</a> 4.1 查询所有角色权限（仅用户自定义角色）</h5><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code> db.runCommand<span class="token punctuation">(</span><span class="token punctuation">{</span> rolesInfo: <span class="token number">1</span> <span class="token punctuation">}</span><span class="token punctuation">)</span>
